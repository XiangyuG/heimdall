...
def normalize_helper_fail_helpers(raw_helpers):
    """Normalize helper names from CLI/config into canonical helper keys."""
    normalized = set()
    if not raw_helpers:
        return normalized
...
class VerificationResult:
    """Structured result from equivalence verification."""
    equivalent: bool
    result_type: str
    counter_example: str = ""
    raw_model: Any = None
...
class Z3VariableUnifier:
    """Renames Z3 variables from two independent angr runs into a unified namespace.

    Categories:
      - Shared inputs: bpf_ctx, ktime, pid_tgid, etc. → same variable in both programs
      - Shared initial map state: map_X_vN → same initial contents
      - Shared key_exists: key_exists_X_vN → same initial existence
      - Distinct outputs: output_r0_c vs output_r0_rust
      - Distinct final maps: final_map_X_vN_c vs final_map_X_vN_rust
      - Internal variables: tagged per-program
    """
    ...
    def normalize_ast(self, claripy_ast, lang_tag):
        """Legacy method: convert claripy AST to unified Z3 AST."""
...
def _collect_z3_vars(z3_ast):
    """Collect all uninterpreted constants (variables) from a Z3 AST."""
    vs = set()
    visited = set()
    def walk(n):
        z3_id = n.get_id()
...
def build_map_ite_for_path(entries_z3, query_key, default_val):
    """Build ITE chain for a single path's map entries.

    Entries are sorted by write_seq so that the last temporal write gets
    highest ITE priority (last-write-wins).
    entries_z3: list of (key_z3, val_z3, exists_z3, written, write_seq) tuples.
    """
...
def build_map_ite_presence_for_path(entries_z3, query_key, default_presence):
    """Build ITE chain for key presence on a single path.

    Returns a 1-bit BV: 1 if query_key is present, else default_presence.
    For array maps (exists_z3 is None), any touched key is considered present.
    For hash maps, presence depends on exists_cond.
    Sorted by write_seq for correct last-write-wins priority.
    """
    sorted_entries = sorted(entries_z3, key=lambda e: e[4])
...
def build_map_ite(paths_z3, map_name, query_key, default_val):
    """Build overall ITE combining all paths for a given map.

    paths_z3: list of (path_pred_z3, dict[map_name → entries_z3])
    """
    result = default_val
...
def build_map_ite_presence(paths_z3, map_name, query_key, default_presence):
    """Build overall presence ITE combining all paths for a given map.

    Returns a 1-bit BV: 1 if query_key is present after execution, else default.
    """
    result = default_presence
    for path_pred_z3, path_map_entries in reversed(paths_z3):
        entries = path_map_entries.get(map_name, [])
...
def verify_equivalence_ite(c_program_data, rust_program_data, unifier,
    ...
    Args:
        c_program_data: ProgramFormula from C program
        rust_program_data: ProgramFormula from Rust program
    ...
    def convert_program_paths(program_data, lang_tag):
        """Convert a ProgramFormula's paths to Z3 with unification.

        Returns:
            - z3_formula: disjunction of (path_pred ∧ output_eq) for all paths
            - paths_z3: list of (path_pred_z3, {map_name: [(key, val, exists)]})
            - output_var_z3: the unified output variable
            - paths_z3_glob: list of (path_pred_z3, {sym_name: val_z3})
        """
    ...
    def _collect_snapshot_bitwidths(program_data, map_name):
        """Collect key/value bit-widths seen in path snapshots for a map."""
        key_bits_set = set()
        val_bits_set = set()
    ...
    def _resolve_map_bitwidths(map_name, c_meta, r_meta):
        """
        Resolve map key/value bit-widths robustly from metadata and snapshots.
        Returns (key_bits, val_bits) or raises ValueError with a detailed message.
        """
        key_candidates = []
        val_candidates = []
    ...
    def _is_global_written(sym_name, paths_z3_glob, bits, lang_tag):
        """Return True if the global was written on any path (final value ≠ initial BVS)."""
    ...
    def _build_glob_ite(sym_name, paths_z3_glob, default_z3):
        """Build ITE chain for a global given a default Z3 expression."""
        ite = default_z3
...
def parse_map_specs(map_specs):
    """Parse map specs from 'name' or 'name:type' format.

    Returns (map_symbol_names, map_type_names) lists.
    """
    map_symbol_names = []
    map_type_names = []
...
class VerificationContext:
    """Cached context for repeated verification against the same C program."""
    shared_vars: dict
...
def generate_c_formula(vctx, c_filepath, entry_sym, max_steps=50000, ringbuf_track_max=512):
    """Generate the C program formula and cache it in the VerificationContext.

    Returns VerificationResult on error, or None on success.
    """
    print("\n" + "=" * 60)
    ...
    try:
        c_formula, _, c_program_data = generate_formula_for_program(
            ...
            symbolic_uninit=vctx.symbolic_uninit,
        )
...
def _check_map_type_consistency(c_filepath, rust_filepath):
    """Strict aggregate-counter check that the C and Rust binaries declare the
    same eBPF map-type distribution.

    Catches translation drifts that the symbex backend would otherwise treat
    as equivalent because output sinks and per-CPU storage are modeled as
    opaque in the per-program formula. Concrete cases observed in practice:
      - C BPF_MAP_TYPE_RINGBUF -> Rust PerfEventArray (e.g., exitsnoop):
        different kernel emission path (ringbuf submit vs perf_event_output)
        and different userspace reader API.
      - C BPF_MAP_TYPE_PERCPU_HASH -> Rust HashMap (e.g., percpu-softirq):
        per-CPU concurrent writes collapse into a single shared slot, losing
        race-free aggregation that the C source relies on.

    Aggregate-counter (rather than name-keyed) because a Rust translation may
    rename the map (e.g., C 'rb' becomes Rust 'events'); the kernel mechanism
    is what matters for observable equivalence, not the symbol name.

    Filters out 'unknown' parser-noise entries from BTF padding / extern
    slots so anonymous map_N stubs don't generate false positives.

    Returns None if consistent, otherwise a VerificationResult with
    result_type='map_type_mismatch' that overrides any UNSAT verdict.
    """
    try:
...
def run_verification_rust_only(vctx, rust_filepath, entry_sym, max_steps=50000, ringbuf_track_max=512):
    """Generate Rust formula and verify against cached C formula.

    Args:
        vctx: VerificationContext with C formula already generated
