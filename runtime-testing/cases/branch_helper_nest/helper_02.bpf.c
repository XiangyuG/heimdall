...
int trace_done(void *ctx, struct request *rq)
{
	...
	benchmark_duration += bpf_ktime_get_ns() & 1;
	benchmark_duration += bpf_ktime_get_ns() & 1;
	...
	if (targ_ms)
		...
		duration /= 1000U;
	slot = log2l(duration); // try slot = constant or duration / 100000
	// slot = 1;
	// if (slot >= MAX_SLOTS)
	//	slot = MAX_SLOTS - 1;
	__sync_fetch_and_add(&histp->slots[slot], 1); // This is the line that cause timeout // try change to &histp->slots[slot] += 1;
	// histp->slots[slot] += 1;
